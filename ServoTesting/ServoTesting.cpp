...
public:
	...
	RobotDemo(void) :
		jFL(5), jRL(6), jFR(3), jRR(4), stick(1), spike(2, Relay::kForwardOnly), 
		gyro(1), mswitch(1)
	{
		printf("Instantiation finished.");
	}
	...
	void OperatorControl(void) {
		...
		while (IsOperatorControl()) {
			...
			//spike.Set(Relay::kOn);
			 if (mswitch.Get() == 0) 
				 screen->PrintfLine(DriverStationLCD::kUser_Line4, "Off");
			 else
				 screen->PrintfLine(DriverStationLCD::kUser_Line4, "On");
			...
			screen->PrintfLine(DriverStationLCD::kUser_Line2,
					"X2 %.3f, Y2 %.3f", x2);
			screen->PrintfLine(DriverStationLCD::kUser_Line3, "Angle %f", angle);
...
//			}
			...
			if (stick.GetRawButton(12)) {
				...
				else if ((theta)-45 < -0.5)
				{
					clockwise = (.00889*absolute(theta-45) + .3);
					screen->PrintfLine(DriverStationLCD::kUser_Line4, "A: %.3f, S: %.3f", theta, clockwise);
				}
				else {
					screen->PrintfLine(DriverStationLCD::kUser_Line4, "PERFECT %f", theta);
					clockwise = 0;
				}
			}
			//screen->PrintfLine(DriverStationLCD::kUser_Line4, "A: %.3f, S: %.3f", theta, clockwise);
